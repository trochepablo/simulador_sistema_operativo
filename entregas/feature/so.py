...
class AbstractInterruptionHandler:
    ...
    def context_swicth(self, pcbcpu, pcbnew):
    ...
    def siTengoQueExpropiarSinoGuardo(self, pcbincpu, newpcb):
        if self._kernel.scheduler.mustExpropiate(pcbincpu, newpcb):
            self.context_swicth(pcbincpu, newpcb)
        else:
...
class SaveInterruptionHandler(AbstractInterruptionHandler):
    def execute(self, irq):
        pcb = self.kernel.pcbTable.runningPcb
        listPages = self.kernel.memoryManager.getPageTable(pcb.pid)
        modPageTable = listPages[math.ceil(pcb.pc / len(listPages))]  # se obtiene pageTable
        self.kernel.memoryManager.updatePageTable(
            pcb.pid, modPageTable.page, modPageTable.frame, False, True
        )
        self.kernel.memoryManager.swapOutIfAplly(pcb.pid, modPageTable.page, modPageTable.frame)
...
class PageFaultIntHandler(AbstractInterruptionHandler):
    ...
    def execute(self, irq):
        ...
        pcb = self.kernel.pcbTable.runningPcb
        self.kernel.loader.loadPageOnDemand(pcb, pageId)
...
class KillInterruptionHandler(AbstractInterruptionHandler):
    ...
    def execute(self, irq):
        ...
        listPages = self.kernel.memoryManager.getPageTable(pcb.pid)
        frameToPush = list(filter(lambda p: p.frame is not None, listPages))
        self.kernel.memoryManager.putFreeFrames(map(lambda p: p.frame, frameToPush))
...
class TimerOutInterruptionHandler(AbstractInterruptionHandler):
    ...
    def execute(self, irq):
        if self.kernel.scheduler.hasNext():
            ...
            self.context_swicth(pcbCPU, nextPCB)
        else:
...
class PageTable:
    def __init__(self, page, frame, bit=False, bitm=False):
        self._page = page
        self._frame = frame
        self._bit = bit
        self._bitModification = bitm
    ...
    @property
    def bitModification(self):
        return self._bitModification


class FIFOReplacementAlgorithms:
    def __init__(self, mm):
        self._memoryManager = mm

    def selectVictim(self):
        for index, (key, value) in enumerate(self._memoryManager.pageTable.items()):
            if any(pt.bit for pt in value):
                return key
...
class MemoryManager:
    def __init__(self, kernel, memorysize, framesize):
        ...
        self._replacementAlgorithms = FIFOReplacementAlgorithms(self)
    ...
    def allocFrame(self):
        if not self.freeFrames:
            self.breakFreeFrame()  # liberar marco
        return self.freeFrames.pop(0)

    def breakFreeFrame(self):
        pidVictim = self._replacementAlgorithms.selectVictim()
        pagetable = list(filter(lambda p: p.bit, self.getPageTable(pidVictim))).pop(0)
        evictFrame = pagetable.frame
        # self.swapOutIfAplly(pidVictim, pagetable.page, pagetable.frame)
        self.updatePageTable(pidVictim, pagetable.page, None, False, False)
        self.putFreeFrames([evictFrame])
    ...
    def removePageTable(self, pid):
        ...
        for index, element in enumerate(pTable):
            framesToRemove.extend(element.frames)
    ...
    def updatePageTable(self, pid, page, frame, bit, bitm=False):
        listPageTable = self.getPageTable(pid)
        for index, pt in enumerate(listPageTable):
            if pt.page == page:
                pt._frame = frame
                pt._bit = bit
                pt._bitModification = bitm
                break

    def isPageModified(self, pid, pageid):
        return next((x for x in self.pageTable.get(pid) if x.page == pageid)).bitModification

    def swapInIfAplly(self, pcb, pageid):
        if self.isPageModified(pcb.pid, pageid):
            instructions = self._kernel.fileSystem.loadInstructions(pcb.pid, pageid)
        else:
            program = self._kernel.fileSystem.load(pcb.path)
            fz = HARDWARE.mmu.getFrameSize()  # frame size
            instructions = program.instructions[pageid * fz: (pageid * fz) + fz]
        return instructions

    def swapOutIfAplly(self, pid, pageid, frame):
        if self.isPageModified(pid, pageid):
            instructions = self.swapInstructions(frame)
            self._kernel.loader.putInVirtualMemory(pid, pageid, instructions)

    def swapInstructions(self, frame):
        fz = HARDWARE.mmu.getFrameSize()
        return HARDWARE.memory.getFrame(frame, fz)
...
class Loader:
    ...
    def load(self, pcb):
        ...
        framesToAlloc = [None] * programFramesSize
        self.buildPageTable(pcb.pid, framesToAlloc)  # se arma page table y se guarda

    def loadPageOnDemand(self, pcb, pageid):
        instructions = self._memoryManager.swapInIfAplly(pcb, pageid)
        frameToAlloc = self._memoryManager.allocFrame()  # se obtienen frame libre
        self.putInstructions(instructions, frameToAlloc)
        self._memoryManager.updatePageTable(pcb.pid, pageid, frameToAlloc, True)
    ...
    def calculateQuantityFrames(self, quantity):
        ...
        if quantity % 2 != 0:  # verificar la paridad  de  la cantidad instrucciones del program (contando el exit)
    ...
    def putInVirtualMemory(self, pid, page, instructions):
        self._fileSystem.saveInstructions(pid, page, instructions)
...
class FileSystem:
    ...
    def saveInstructions(self, pid, pageid, instructions):
        self._virtualMemory[pid, pageid] = instructions

    def loadInstructions(self, pid, pageid):
        return self._virtualMemory.get(pid, pageid)
...
class Dispatcher:
    ...
    def load(self, pcb, page_table_del_pcb):
        ...
        for index, element in enumerate(page_table_del_pcb):
            HARDWARE.mmu.setPageFrame(element.page, element.frame, element.bit)
...
class Kernel:
    ...
    def __init__(self):
        ...
        trapHandler = PageFaultIntHandler(self)
        HARDWARE.interruptVector.register(TRAP_INTERRUPTION_TYPE, trapHandler)

        saveHandler = SaveInterruptionHandler(self)
        HARDWARE.interruptVector.register(SAVE_INTERRUPTION_TYPE, saveHandler)
