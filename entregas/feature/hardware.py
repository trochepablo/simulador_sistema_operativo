...
class ASM:
    ...
    @classmethod
    def isSave(self, instruction):
        return INSTRUCTION_SAVE == instruction
...
TRAP_INTERRUPTION_TYPE = "#TRAP"
SAVE_INTERRUPTION_TYPE = "#SAVE"
...
class Memory():
    ...
    def getFrame(self, frame, frameSize):
        return self._cells[frame: frame + frameSize]
...
class MMU():
    ...
    def getMemorySize(self):
        return len(self._memory._cells)

    def resetTLB(self):
        self._tlb = dict()

    def setPageFrame(self, pageId, frameId, bit):
        self._tlb[pageId] = TBL(frameId, bit)
    ...
    def fetch(self,  logicalAddress):
        ...
        try:
            tlbPage = self._tlb[pageId]
            if not tlbPage.bit:
                trapIRQ = IRQ(TRAP_INTERRUPTION_TYPE, pageId)
                self._interruptVector.handle(trapIRQ)
        except:
            raise Exception("\n*\n* ERROR \n*\n Error en el MMU\nNo se cargo la pagina  {pageId}".format(pageId = str(pageId)))

        # after to update page table
        tlbPage = self._tlb[pageId]

        # calculamos la direccion fisica resultante
        frameBaseDir = self._frameSize * tlbPage.frame
...
class Cpu():
    ...
    def _execute(self):
        ...
        elif ASM.isSave(self._ir):
            saveIRQ = IRQ(SAVE_INTERRUPTION_TYPE)
            self._interruptVector.handle(saveIRQ)
        else:
...
class Hardware():
    ...
    def setup(self, memorySize):
        ...
        self._mmu = MMU(self._memory, self._interruptVector)
