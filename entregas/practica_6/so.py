...
class TrapInterruptionHandler(AbstractInterruptionHandler):
...
class PageTable:
    def __init__(self, page, frame, bit=False):
        self._page = page
        self._frame = frame
        self._bit = bit
...
class MemoryManager:
    ...
    def updatePageTable(self, pid, page, frame, bit):
...
class Loader:
    ...
    def loadPageOnDemand(self, pcb, pageid):
        program = self._fileSystem.load(pcb.path)  # lectura de programa
        frameToAlloc = self.getAllocFrames(1).pop()  # se obtienen frame libre
        fz = self.getFrameSize()  # frame size
        self.putInstructions(program.instructions[pageid * fz: (pageid * fz) + fz], frameToAlloc)
...
class Kernel:
    ...
    def __init__(self):
        ...
        trapHandler = TrapInterruptionHandler(self)
